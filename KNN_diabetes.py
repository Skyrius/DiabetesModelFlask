...
#using with flask so don't need to plot- but will need to run with ipython from command line
#if using this later as a standalone
...
#******NOTE: Seaborn version problem, pandas auto-generated dataframe types isn't playing nice, currently depreciated so it'll need to be manually generated to use the plot
## data type analysis
...
#importing train_test_split***************************** Start here for saving models
...
## let's try saving and loading the file with pickle and testing a random person's values- 
## this is for knn without the grid search- another model knn_cv below has grid search
pickle.dump(knn, open('KNN_diabetes.pkl','wb'))
...
# model for gridsearchcv- note because gridsearch is essentially testing several different version of a model with different
# parameters, if anything in the model changes, this needs to be recompiled with the new model, so this is ONLY for this current
# KNN model
pickle.dump(knn_cv, open('KNNcv_diabetes.pkl','wb'))


# In[46]:


modelcv = pickle.load(open('KNNcv_diabetes.pkl','rb'))
